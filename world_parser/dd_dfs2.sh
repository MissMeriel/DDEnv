...
#orig_failure=$script_dirname/failures/rubble11_brick3.log
#orig_failure=$script_dirname/failures/frictionpatch6_origfailure2.log
orig_failure=$script_dirname/failures/rough_terrain_static_origfailure_correct1.log
...
while [[ "${min_world_reached}" != "true" && $clusters -le $models ]]; do
   ...
   elif [[ $partitioning == 'trajectory_seg' && $ordering == 'avg_trajectory' ]]; then
      #TODO: DEBUG
      python3 $script_dirname/partition_and_order.py $world_name -d ${target_world_dir} -tseg $posefile -atd $posefile -nsegs $clusters
   fi
...
echo "World reductions per iteration:"
for value in "${reduction_array[@]}"
do
     echo $value
done
